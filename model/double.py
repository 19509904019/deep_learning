...
for i in range(len(arr)):
    ...
    sCommand = ['With Brick',
                '.Reset',
                '.Name "%s"' % 'middle_layer_2',
                '.Component "%s"' % 'component1',
                '.Material "%s"' % 'medium',
                f'.Xrange "-p/2", "p/2"',
                f'.Yrange "-p/2", "p/2"',
                f'.Zrange "h+0.018", "h+0.018+h"',
                '.Create',
                'End With']
    sCommand = line_break.join(sCommand)
    modeler.add_to_history('define brick', sCommand)
    ...
    for x in range(a.shape[0]):
        for y in range(a.shape[1]):
            if a[x][y] == 1:
                ...
                sCommand = ['With Brick',
                            '.Reset',
                            '.Name "2metal_%.0f_%.0f"' % (x + 1, y + 1),
                            '.Component "component1"',
                            '.Material "Copper (annealed)"',
                            '.Xrange "%f","%f"' % (y * 0.8 - (p / 2), y * 0.8 - (p / 2) + t),
                            '.Yrange "%f","%f"' % ((p / 2) - (x * 0.8 + t), (p / 2) - x * 0.8),
                            '.Zrange "h+0.018+h","h+0.018+h+0.018"',
                            '.Create',
                            'End With']
                sCommand = line_break.join(sCommand)
                modeler.add_to_history('define brick', sCommand)
                ...
                sCommand = ['With Transform',
                            '.Reset',
                            '.Name "component1:2metal_%.0f_%.0f"' % (x + 1, y + 1),
                            '.Origin "Free"',
                            '.Center "0", "0", "0"',
                            '.PlaneNormal "1", "0", "0"',
                            '.MultipleObjects "True"',
    ...
    sCommmd = [r'SelectTreeItem("1D Results\S-Parameters\S1,1")',
               ...
               'With ASCIIExport',
               '.Reset',
               '.FileName "%s"' % rf'C:\Users\Dell\Desktop\phase1\{count}-phase.txt',
               '.Execute',
               'End With']
    sCommmd = '\n'.join(sCommmd)
    ...
    sCommmd = [r'SelectTreeItem("1D Results\S-Parameters\S1,1")',
               ...
               'With ASCIIExport',
               '.Reset',
               '.FileName "%s"' % rf'C:\Users\Dell\Desktop\linear1\{count}-linear.txt',
               '.Execute',
               'End With']
    sCommmd = '\n'.join(sCommmd)
